...
class AesCtrPrng(SecureBatchPrng):
    ...
    def GenerateBytes(self,cache,cache_size):
        if (cache_size % kBlockSize) != 0 :
            print("Number of bytes generated by AesCtrPrng must be a multiple of "+ str(kBlockSize))
        if cache_size > kCacheSize :
            print("Requested number of bytes "+str(cache_size)+"exceeds maximum cache size " +str(kCacheSize))
        if self.blocks_generated > kMaxBlocks :
            print("AesCtrPrng generated " +str(kMaxBlocks) + "blocks and needs a new seed.")
        k = kAllZeroes[:cache_size]
